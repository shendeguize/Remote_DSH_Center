...
export function createHostTable({ store, actions }) {
  ...
  function renderActions(host) {
    ...
    return rowActions(host).map((action) => {
      if (action === 'open') {
        return button(ACTION_LABEL.open, {
          variant: 'primary',
      ...
      return button(ACTION_LABEL[action], {
  ...
  const offs = [
    store.on('hosts:reset', renderAll),
    store.on('hosts:changed', renderOne),
    store.on('pending:changed', renderAll),
    store.on('connection:changed', renderAll),
  ];
  renderAll();
