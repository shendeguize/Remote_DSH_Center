...
改动与其他注入项同规矩：**保存后下次拉起生效**，不动正在跑的实例（页面会提示重启该主机的
`dsh web`；只重启 manager 不会切换存活实例的工作目录）。

SSH 映射只把页面带到本机，不会把远端桌面能力也带回来。远端 `dsh web` 里的「打开配置文件」
仍是在目标主机调用桌面打开器。无桌面的 Linux 主机可在管理页打开该主机详情，使用
「dsh 配置文件」区块经 SSH 读取和编辑目标账户的
`${DSH_HOME:-$HOME/.dsh}/settings.yaml`；本机条目走等价的本机通道。Center 把文件当作
不透明 UTF-8 文本，不解析或改写 YAML 字段，因此不会绑死 dsh 的当前配置 schema。
读取和写入上限均为 512 KiB；保存前会用校验和确认文件没有被其他编辑器改过，再备份并原子替换。
冲突或保存结果无法确认时，页面会要求重新加载，且保留原草稿供手工合并。当前 dsh 会自行监视并
...
- 远端数据面使用 `ssh -L`，加密与鉴权都由你的 ssh 配置与密钥负责；本机数据面直连环回端口，
  不经过 SSH。使用「dsh 配置文件」时，文件可能包含凭据：正文只短暂停留在 manager/浏览器
  内存与一次性命令 stdin，不写入 manager 配置、日志或 SSE；因此也不要向不受信任的人开放
  manager 页面或浏览器会话。
- 除用户明确点击「保存文件」外，本机与远端受管侧的运行落地物都限制在各自 HOME 下的
  `~/.dsh_center_remote/`（日志、patch、settings staging 与单份备份）。这项显式操作只允许写
  解析后的 `${DSH_HOME:-$HOME/.dsh}/settings.yaml`，API 不接受任意路径。本机 patch 同步不会
  cleanup 无法确认归属的既有文件，目标路径也被限制在该目录内。
